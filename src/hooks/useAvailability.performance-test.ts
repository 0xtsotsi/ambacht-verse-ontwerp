...
describe("useAvailability Performance Comparison", () => {
  ...
  it("should demonstrate performance improvements in date checking operations", () => {
    const { result: originalResult } = renderHook(() =>
      useAvailabilityOriginal(),
    );
    const { result: optimizedResult } = renderHook(() =>
      useAvailabilityOptimized(),
    );
  ...
  it("should maintain the same API surface", () => {
    ...
    expect(typeof originalResult.current.checkSlotAvailability).toBe(
      "function",
    );
    expect(typeof optimizedResult.current.checkSlotAvailability).toBe(
      "function",
    );
