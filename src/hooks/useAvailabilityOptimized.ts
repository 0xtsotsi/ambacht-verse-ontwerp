...
function createSlotsHash(slots: AvailabilitySlot[]): string {
  return slots.map(s => `${s.date}_${s.time_slot}_${s.current_bookings}_${s.max_bookings}`).join('|');
}
...
function mapAvailabilityToDateCheckerOptimized(
  ...
  dateMap.forEach((slots, date) => {
    ...
    const someLimited = slots.some(s => s.current_bookings >= s.max_bookings - 1 && s.current_bookings < s.max_bookings);
