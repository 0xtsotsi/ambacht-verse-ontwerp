...
describe('Service Tier Constants Validation', () => {
  it('should have correct SERVICE_TIERS configuration', () => {
    ...
    expect(premium?.priceMultiplier).toBe(1.00);
...
describe('Component Import Validation', () => {
  it('should import ServiceTierSystem without errors', async () => {
    const { ServiceTierSystem } = await import('@/components/ServiceTierSystem');
    expect(ServiceTierSystem).toBeDefined();
  });

  it('should import sub-components without errors', async () => {
    const { ServiceTierMatrix } = await import('@/components/ServiceTierMatrix');
    const { ServiceTierComparison } = await import('@/components/ServiceTierComparison');
    const { ServiceTierCalculator } = await import('@/components/ServiceTierCalculator');
