...
export type AddOnService =
  Database["public"]["Tables"]["add_on_services"]["Row"];
export type BookingAddOn =
  Database["public"]["Tables"]["booking_add_ons"]["Row"];
...
export async function getAvailabilitySlots(
  ...
  try {
    ...
    return (
      (await withSupabaseLogging(query, "availability_slots", "GET")) ?? []
    );
  } catch (error) {
...
export async function createBooking(
  ...
  try {
    ...
    const query = supabase.from("bookings").insert(booking).select().single();

    const data = await withSupabaseLogging(query, "bookings", "POST");
...
export async function getBooking(id: string): Promise<Booking | null> {
  try {
    const query = supabase.from("bookings").select("*").eq("id", id).single();

    return await withSupabaseLogging(query, "bookings/get", "GET");
  } catch (error) {
...
export async function updateBookingStatus(
  ...
  try {
    ...
    const data = await withSupabaseLogging(query, "bookings/update", "PATCH");
...
export async function createQuote(quote: QuoteInsert): Promise<Quote | null> {
  try {
    const query = supabase.from("quotes").insert(quote).select().single();

    return await withSupabaseLogging(query, "quotes", "POST");
  } catch (error) {
...
export async function getQuotesForBooking(bookingId: string): Promise<Quote[]> {
  try {
    ...
    return (await withSupabaseLogging(query, "quotes/booking", "GET")) ?? [];
  } catch (error) {
...
export async function updateQuoteStatus(
  ...
  try {
    ...
    return await withSupabaseLogging(query, "quotes/update", "PATCH");
  } catch (error) {
...
export async function getAddOnServices(): Promise<AddOnService[]> {
  try {
    ...
    return (await withSupabaseLogging(query, "add_on_services", "GET")) ?? [];
  } catch (error) {
...
export async function getAddOnServicesByCategory(
  category: string,
  ...
  try {
    ...
    return (
      (await withSupabaseLogging(query, "add_on_services/category", "GET")) ??
      []
    );
  } catch (error) {
...
export async function addBookingAddOns(
  ...
  try {
    ...
    return (
      (await withSupabaseLogging(insertQuery, "booking_add_ons", "POST")) ?? []
    );
  } catch (error) {
  ...
  return {
    dates: {
      booked: Array.from(dateMap.entries())
        .filter(([_, slots]) =>
          slots.every((s) => s.current_bookings >= s.max_bookings),
        )
    ...
    timeSlots: {
      morning: slots
        .filter((s) => s.time_slot >= "10:00" && s.time_slot < "12:00")
        .map((s) => s.time_slot),
      afternoon: slots
        .filter((s) => s.time_slot >= "12:00" && s.time_slot < "16:00")
        .map((s) => s.time_slot),
      evening: slots
        .filter((s) => s.time_slot >= "16:00" && s.time_slot <= "20:00")
        .map((s) => s.time_slot),
    },
  };
}
