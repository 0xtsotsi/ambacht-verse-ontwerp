...
export class ValidationService {
  ...
  validateValue<T>(value: T, rules: ValidationRule<T>[]): ValidationResult {
    ...
    try {
      ...
      SafeLogger.error('Validation error:', error, { value, rules: rules.map(r => r.name) });
      return {
        isValid: false,
  ...
  validatePhoneNumber(phone: string): ValidationResult {
    const rules: ValidationRule<string>[] = [
      ...
      {
        name: 'phone_format',
        validate: (value) => /^[\+]?[0-9\s\-\(\)]{10,}$/.test(value.replace(/\s/g, '')),
   ...
   */
  validateGuestCount(count: number, serviceType: string = 'corporate'): ValidationResult {
