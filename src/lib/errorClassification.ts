...
export function classifyError(
  ...
  const recoverySuggestion = generateRecoverySuggestion(type, status, fullContext);
 ...
 */
function extractErrorCode(error: any): string | number | undefined {
  return error?.code || 
 ...
 */
function determineErrorType(error: any, status?: number, message?: string): ErrorType {
 ...
 */
function determineErrorCategory(type: ErrorType, status?: number): ErrorCategory {
  switch (type) {
    case 'network':
    case 'timeout':
      return 'connectivity';
 ...
 */
function determineErrorSeverity(type: ErrorType, status?: number): ErrorSeverity {
  switch (type) {
    case 'authentication':
    case 'authorization':
      return 'high';
