...
export function sanitizeData(
  ...
  if (Array.isArray(data)) {
    return data.map(item => sanitizeData(item, { ...options, maxDepth: maxDepth - 1 }));
  }
  ...
  if (typeof data === 'object') {
    ...
    for (const [key, value] of Object.entries(data)) {
      ...
      if (removeFields.some(field => lowerKey.includes(field.toLowerCase()))) {
      ...
      sanitized[key] = sanitizeData(value, { ...options, maxDepth: maxDepth - 1 });
    }
 ...
 */
export function sanitizeError(error: any, context?: Record<string, any>): Record<string, any> {
...
export const SafeLogger = {
  ...
  error: {
    // Overload for Error objects to preserve stack trace type information
    (message: string, error: Error, context?: Record<string, any>): void;
    (message: string, error?: any, context?: any): void;
    (message: string, error?: any, context?: any): void {
