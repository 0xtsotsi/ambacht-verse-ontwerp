...
export const ComponentLogger = {
   ...
   */
  performance: (componentName: string, renderTime: number, isSlowRender: boolean) => {
...
export const APILogger = {
   ...
   */
  error: (endpoint: string, error: Error, retryAttempt: number = 0, requestId?: string) => {
   ...
   * Log retry attempts
   */
  retry: (endpoint: string, attempt: number, maxAttempts: number, delay: number, requestId?: string) => {
...
export const UserFlowLogger = {
   ...
   * Log user navigation
   */
  navigation: (from: string, to: string, sessionId: string, userId?: string) => {
