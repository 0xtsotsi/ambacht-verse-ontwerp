...
describe('LoggerUtils', () => {
  ...
  describe('SafeLogger', () => {
    ...
    it('should log safely with data sanitization', () => {
      ...
      expect(consoleSpy).toHaveBeenCalledWith('Test message', expect.objectContaining({
        message: 'User action',
  ...
  describe('performance and edge cases', () => {
    ...
    it('should handle complex nested structures', () => {
      ...
      expect(sanitized.users[0].profile.personal.email).toMatch(/\*+@test\.com/);
      expect(sanitized.users[0].profile.personal.contacts.phone).toMatch(/\d{2}\*+\d{2}/);
