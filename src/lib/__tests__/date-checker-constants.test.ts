...
describe("date-checker-constants", () => {
  ...
  describe("calculateEstimatedPrice", () => {
    it("should calculate correct price for valid categories", () => {
      const corporatePrice = calculateEstimatedPrice(
        "corporate",
        "premium",
        20,
      );
      const expectedPrice =
        SERVICE_CATEGORIES.corporate.basePrice! *
        SERVICE_TIERS.premium.multiplier *
        20;
    ...
    describe("price calculation accuracy", () => {
      it("should calculate corporate pricing correctly", () => {
        ...
        const premiumPrice = calculateEstimatedPrice(
          "corporate",
          "premium",
          50,
        );
    ...
    describe("memoization behavior simulation", () => {
      it("should produce consistent results for same inputs", () => {
        const params: [ServiceCategory, ServiceTier, number] = [
          "social",
          "premium",
          75,
        ];
        ...
        expect(firstResult).toBe(
          SERVICE_CATEGORIES.social.basePrice! * 1.4 * 75,
        );
      });
      ...
      it("should handle category and tier changes correctly", () => {
        const testCases: Array<[ServiceCategory, ServiceTier, number, number]> =
          [
            ["corporate", "basis", 50, 12.5 * 1.0 * 50],
            ["corporate", "premium", 50, 12.5 * 1.4 * 50],
            ["social", "basis", 50, 27.5 * 1.0 * 50],
            ["social", "luxe", 50, 27.5 * 1.8 * 50],
            ["wedding", "premium", 50, 22.5 * 1.4 * 50],
    ...
    describe("floating point precision", () => {
      it("should handle decimal calculations correctly", () => {
        ...
        const expectedPrice = 27.5 * 1.4 * 13; // 500.50
      ...
      it("should maintain precision with large numbers", () => {
        ...
        const expectedPrice = 12.5 * 1.8 * 333; // 7492.50
  ...
  describe("constants validation", () => {
    ...
    it("should have valid guest count config", () => {
      ...
      expect(GUEST_COUNT_CONFIG.default).toBeGreaterThanOrEqual(
        GUEST_COUNT_CONFIG.min,
      );
      expect(GUEST_COUNT_CONFIG.default).toBeLessThanOrEqual(
        GUEST_COUNT_CONFIG.max,
      );
    ...
    it("should have consistent service categories and tiers", () => {
      ...
      tiers.forEach((tier) => {
        expect(
          SERVICE_TIERS[tier as keyof typeof SERVICE_TIERS].multiplier,
        ).toBeGreaterThan(0);
      });
    });
  });
  ...
  describe("translation fallback mechanism", () => {
    ...
    describe("createSafeTranslations", () => {
      it("should create proxy with fallback behavior", () => {
        ...
        expect(safeTranslations.confirm).toBe(
          DATE_CHECKER_TRANSLATIONS.nl.confirm,
        );

        // Test non-existent key fallback
        expect(
          (safeTranslations as Record<string, unknown>).nonExistentKey,
        ).toBe("Non Existent Key");
      });
      ...
      it("should work with both Dutch and English", () => {
        ...
        expect((dutchSafe as Record<string, unknown>).missingKey).toBe(
          "Missing Key",
        );
        expect((englishSafe as Record<string, unknown>).missingKey).toBe(
          "Missing Key",
        );
      });
    });
    ...
    describe("createSafeNavigationLabels", () => {
      it("should create proxy with navigation fallback behavior", () => {
        ...
        // Test fallback
        expect((safeNavigation as Record<string, unknown>).nonExistentNav).toBe(
          "Non Existent Nav",
        );
      });
      ...
      it("should handle complex navigation labels", () => {
        ...
        expect(safeNavigation.screenReaderStepAnnouncement).toBe(
          NAVIGATION_LABELS.en.screenReaderStepAnnouncement,
        );

        expect(safeNavigation.bookingConfirmedSuccess).toBe(
          NAVIGATION_LABELS.en.bookingConfirmedSuccess,
        );
      });
    });
    ...
    describe("translation completeness validation", () => {
      it("should have all required translation keys for both languages", () => {
        ...
        requiredKeys.forEach((key) => {
          ...
          expect(
            typeof DATE_CHECKER_TRANSLATIONS.nl[
              key as keyof typeof DATE_CHECKER_TRANSLATIONS.nl
            ],
          ).toBe("string");
          expect(
            typeof DATE_CHECKER_TRANSLATIONS.en[
              key as keyof typeof DATE_CHECKER_TRANSLATIONS.en
            ],
          ).toBe("string");
        });
      });
      ...
      it("should have all required navigation labels for both languages", () => {
        ...
        requiredNavKeys.forEach((key) => {
          ...
          expect(
            typeof NAVIGATION_LABELS.nl[
              key as keyof typeof NAVIGATION_LABELS.nl
            ],
          ).toBe("string");
          expect(
            typeof NAVIGATION_LABELS.en[
              key as keyof typeof NAVIGATION_LABELS.en
            ],
          ).toBe("string");
        });
      });
      ...
      it("should have consistent placeholder patterns in navigation labels", () => {
        ...
        const dutchBookingSuccess =
          NAVIGATION_LABELS.nl.bookingConfirmedSuccess;
        const englishBookingSuccess =
          NAVIGATION_LABELS.en.bookingConfirmedSuccess;

        expect(dutchBookingSuccess).toContain("{count}");
        expect(englishBookingSuccess).toContain("{count}");

        const dutchStepAnnouncement =
          NAVIGATION_LABELS.nl.screenReaderStepAnnouncement;
        const englishStepAnnouncement =
          NAVIGATION_LABELS.en.screenReaderStepAnnouncement;
  ...
  describe("Dutch Holiday Generation", () => {
    ...
    describe("getBlackoutDates", () => {
      it("should return holidays for current and next year", () => {
        ...
        const currentYearDates = blackoutDates.filter((date) =>
          date.startsWith(`${currentYear}-`),
        );
        const nextYearDates = blackoutDates.filter((date) =>
          date.startsWith(`${currentYear + 1}-`),
        );
