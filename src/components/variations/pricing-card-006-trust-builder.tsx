...
const PricingCard006TrustBuilder: React.FC<PricingCardProps> = ({
  ...
  const trustTranslations = {
    nl: { trusted: 'Vertrouwd', satisfied: '98% Tevreden', since: 'Sinds 1989', guarantee: '100% Betrouwbaarheidsgarantie', testimonial: 'Geweldige service, precies wat we nodig hadden!', reviewer: 'Maria van der Berg' },
    en: { trusted: 'Trusted', satisfied: '98% Satisfied', since: 'Since 1989', guarantee: '100% Reliability Guarantee', testimonial: 'Great service, exactly what we needed!', reviewer: 'Maria van den Berg' }
  };
  ...
  return (
    <Card className={`relative ${colorScheme.gradient} ${COMMON_STYLES.card} transition-all duration-300 hover:scale-105`} data-testid="pricing-card">
      ...
      <CardHeader className="pb-4">
        ...
        <div className="space-y-2 mt-3">
          ...
          <p className="text-xs text-gray-600">{tt.satisfied} • {tt.since}</p>
        </div>
        ...
        <div className={`text-center py-4 ${COMMON_STYLES.priceDisplay} mt-4`}>
          {finalPrice ? (
            ...
            <div className="text-2xl font-bold text-[#CC5D00]">{t.getQuote}</div>
          )}
        </div>
      </CardHeader>
