  ...
  return (
    <div className="w-full max-w-6xl mx-auto p-6 space-y-8">
      ...
      <div className="text-center space-y-4">
        ...
        <p className="text-lg text-forest-green/80 max-w-2xl mx-auto">{t.subtitle}</p>
      </div>
            ...
            <Card className={QUOTE_CALCULATOR_STYLES.card}>
              ...
              <CardContent className="space-y-4">
                <div className="text-center">
                  <span className="text-2xl font-bold">{guestCount[0]} {t.guests}</span>
                </div>
                <Slider
                  value={guestCount}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {Object.entries(QUOTE_ADD_ONS).map(([key, addon]) => (
                  <div key={key} className={cn(
                    ...
                    selectedAddOns.includes(key as QuoteAddOn) ? QUOTE_CALCULATOR_STYLES.addOnSelected : QUOTE_CALCULATOR_STYLES.addOnDefault
                  )}>
                    <div className="flex items-center space-x-3">
                      <Checkbox
                        id={key}
                        checked={selectedAddOns.includes(key as QuoteAddOn)}
                        onCheckedChange={() => handleAddOnToggle(key as QuoteAddOn)}
                      />
                      <div>
                        <Label htmlFor={key} className="font-medium cursor-pointer">
                          {addon.label}
                          {addon.popular && <Badge className="ml-2 text-xs">{t.popular}</Badge>}
                        </Label>
                ...
                <>
                  <div className="space-y-2">
                    <div className="flex justify-between">
                      ...
                      <span className="text-sm">€{quote.basePrice.toFixed(2)}</span>
                    </div>
                    {quote.addOnTotal > 0 && (
                      <div className="flex justify-between">
                        <span className="text-sm">{t.addOnTotal}:</span>
                        <span className="text-sm">€{quote.addOnTotal.toFixed(2)}</span>
                      </div>
                    )}
                    <div className="flex justify-between">
