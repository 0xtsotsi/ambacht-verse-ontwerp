...
test.describe('Complete Booking Flow Integration', () => {
  ...
  test('complete booking journey from widget to form submission', async ({ page }) => {
    ...
    await page.fill('textarea[placeholder="Bericht"]', 'Ik wil graag een reservering maken voor 20 personen.');
  ...
  test('multi-language support verification', async ({ page }) => {
    ...
    await expect(page.locator('input[placeholder="E-mailadres"]')).toBeVisible();
  ...
  test('performance: widget should not block page interactions', async ({ page }) => {
    ...
    if (widgetBox && navBox) {
      const overlaps = !(widgetBox.x + widgetBox.width < navBox.x || 
