...
test.describe('Quote Calculator Accessibility', () => {
  ...
  test.describe('Keyboard Navigation', () => {
    ...
    test('should provide clear focus indicators', async () => {
      ...
      const ring = await focusedElement.evaluate(el => getComputedStyle(el).getPropertyValue('--tw-ring-width'));
    ...
    test('should trap focus within modal', async () => {
      ...
      const tabbableElements = await page.locator('[role="dialog"] [tabindex]:not([tabindex="-1"]), [role="dialog"] button:not([disabled]), [role="dialog"] input:not([disabled]), [role="dialog"] select:not([disabled]), [role="dialog"] textarea:not([disabled])').count();
  ...
  test.describe('Screen Reader Support', () => {
    ...
    test('should announce step changes', async () => {
      ...
      const stepHeading = page.locator('h1, h2, h3, h4, h5, h6').filter({ hasText: 'Selecteer Service Type' });
      ...
      const step2Heading = page.locator('h1, h2, h3, h4, h5, h6').filter({ hasText: 'Kies Service Niveau' });
    ...
    test('should provide meaningful button labels', async () => {
      ...
      const closeButton = page.locator('button[aria-label*="Sluit"], button:has-text("Sluiten")');
      if (await closeButton.count() > 0) {
        const closeButtonLabel = await closeButton.first().getAttribute('aria-label');
  ...
  test.describe('Visual Accessibility', () => {
    test('should have adequate color contrast', async () => {
      ...
      const headingColor = await headingElement.evaluate(el => getComputedStyle(el).color);
  ...
  test.describe('Touch Accessibility', () => {
    ...
    test('should have touch-friendly target sizes', async () => {
      ...
      const buttons = page.locator('button, label[for]').filter({ hasText: /\w+/ });
    ...
    test('should handle touch interactions correctly', async () => {
      ...
      const categoryOption = page.locator('label:has-text("Zakelijke Catering")');
      ...
      const serviceTierOptions = page.locator('label[for]').filter({ hasText: /Essential|Premium|Luxury/ });
  ...
  test.describe('Error State Accessibility', () => {
    ...
    test('should handle loading states accessibly', async () => {
      ...
      if (await loadingElement.count() > 0) {
        const ariaLive = await loadingElement.evaluate(el => 
