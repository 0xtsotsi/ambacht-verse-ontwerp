...
test.describe("Database Integration Tests", () => {
  ...
  test("availability slots are loaded from database", async ({ page }) => {
    ...
    const bookingButton = page.locator("button", {
      hasText: "Beschikbaarheid Controleren",
    });
  ...
  test("time slots are dynamically loaded for selected date", async ({
    page,
  }) => {
    // Open the DateChecker modal
    const bookingButton = page.locator("button", {
    ...
    const timeSlotSections = page.locator(
      "text=Ochtend, text=Middag, text=Avond",
    );
    await expect(timeSlotSections.first()).toBeVisible();
  });

  test("quote calculator integrates with database services", async ({
    page,
  }) => {
    // Navigate to quote calculator (assuming it's accessible)
    const quoteButton = page.locator("button", {
      hasText: "Offerte Calculator",
    });
    ...
    const quoteRequestButton = page.locator("button", {
      hasText: "Detailofferte Aanvragen",
    });
  ...
  test("booking creation works end-to-end", async ({ page }) => {
    ...
    const confirmButton = page.locator("button", {
      hasText: "Bevestig Aanvraag",
    });
    await confirmButton.click();

    // Verify success message
    await expect(page.locator("text=Reservering aangemaakt!")).toBeVisible({
      timeout: 5000,
    });
  });
  ...
  test("error handling works correctly", async ({ page }) => {
    ...
    await expect(
      page.locator("text=Fout bij laden van beschikbaarheid"),
