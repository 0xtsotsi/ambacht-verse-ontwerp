...
test.beforeEach(async ({ page }) => {
  ...
  await page.addInitScript(() => {
    (window as Record<string, unknown>).mockBookingHandler = (
      serviceType: string,
      tier: string,
    ) => {
      (window as Record<string, unknown>).bookingCalls =
        (window as Record<string, unknown>).bookingCalls || [];
      (
        (window as Record<string, unknown>).bookingCalls as Array<{
          serviceType: string;
          tier: string;
        }>
      ).push({ serviceType, tier });
    };
  });
});

test.describe("Pricing Cards - Dutch Transparency (005)", () => {
  test("should display transparent pricing with Dutch cultural elements", async ({
    page,
  }) => {
    ...
    await expect(
      page.locator('[data-testid="transparent-pricing"]'),
    ).toBeVisible();
    ...
    // Test pricing format
    await expect(
      page.locator("text=/Vanaf €\\d+\\.\\d+ per persoon/"),
    ).toBeVisible();
    ...
    await expect(pricingElement).toHaveCSS(
      "color",
      /rgb\(204, 93, 0\)|#CC5D00/,
    );
  });
...
test.describe("Pricing Cards - Trust Builder (006)", () => {
  test("should display trust signals and social proof", async ({ page }) => {
    ...
    await expect(
      page.locator("text=100% Betrouwbaarheidsgarantie"),
    ).toBeVisible();
...
test.describe("Pricing Cards - Mobile First (007)", () => {
  test("should be optimized for mobile devices", async ({ page }) => {
    ...
    await expect(
      page.locator('[data-testid="secondary-actions"]'),
    ).toBeVisible();
  });
...
test.describe("Pricing Cards - Conversion Optimized (008)", () => {
  test("should display urgency and conversion elements", async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="urgency-badge"]')).toHaveClass(
      /animate-pulse/,
    );
  ...
  test("should show social proof variant", async ({ page }) => {
    ...
    await expect(
      page.locator("text=/\\d+ mensen bekijken dit nu/"),
    ).toBeVisible();
  ...
  test("should display scarcity messaging", async ({ page }) => {
    ...
    await expect(
      page.locator('[data-testid="scarcity-indicator"]'),
    ).toBeVisible();
  });
  ...
  test("should show value proposition", async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="original-price"]')).toHaveClass(
      /line-through/,
    );
    await expect(
      page.locator('[data-testid="discounted-price"]'),
    ).toBeVisible();
  });
...
test.describe("Pricing Cards - Premium Positioning (009)", () => {
  ...
  test("should show premium inclusions", async ({ page }) => {
    ...
    await expect(
      page.locator("text=Optional Helicopter Service"),
    ).toBeVisible();
...
test.describe("Cross-Variation Testing", () => {
  test("should maintain consistent pricing across all variations", async ({
    page,
  }) => {
    const testData = [
      { service: "corporate", tier: "basis", expectedBase: 12.5 },
      { service: "social", tier: "premium", expectedBase: 27.5 },
      { service: "wedding", tier: "luxe", expectedBase: 22.5 },
    ];

    for (const variation of pricingCardVariations) {
      for (const data of testData) {
        await page.goto(
          `/pricing-cards-test?variant=${variation}&service=${data.service}&tier=${data.tier}`,
        );
  ...
  test("should maintain accessibility across all variations", async ({
    page,
  }) => {
    for (const variation of pricingCardVariations) {
...
test.describe("Performance Testing", () => {
  test("should load all variations within performance budgets", async ({
    page,
  }) => {
    for (const variation of pricingCardVariations) {
...
test.describe("Integration Testing", () => {
  test("should integrate with booking flow", async ({ page }) => {
    ...
    const calls = (await page.evaluate(
      () => (window as Record<string, unknown>).bookingCalls,
    )) as Array<{ serviceType: string; tier: string }>;
    expect(calls).toHaveLength(1);
...
test.describe("Empathy & Cultural Validation", () => {
  ...
  test("should reduce decision anxiety through clear presentation", async ({
    page,
  }) => {
    await page.goto("/pricing-cards-test?variant=006");
