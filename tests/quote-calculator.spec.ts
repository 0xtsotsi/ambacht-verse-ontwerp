...
test.describe("Preliminary Quote Calculator", () => {
  ...
  test("should open quote calculator from floating widget", async () => {
    ...
    await expect(
      page.locator("text=Welk type evenement organiseert u?"),
    ).toBeVisible();
  });
  ...
  test.describe("Step-by-Step Quote Calculator Flow", () => {
    ...
    test("should complete full quote calculation flow", async () => {
      ...
      await expect(
        page.locator('button:has-text("Detailofferte Aanvragen")'),
      ).toBeVisible();
      await expect(
        page.locator('button:has-text("Detailofferte Aanvragen")'),
      ).toBeEnabled();
    ...
    test("should calculate and display pricing correctly", async () => {
      ...
      await expect(
        page.locator("text=Uw offerte wordt berekend..."),
      ).toBeVisible();
      await page.waitForTimeout(1000);
  ...
  test.describe("Quote Calculator Accessibility", () => {
    ...
    test("should have proper ARIA labels", async () => {
      ...
      await expect(page.locator('[aria-label*="service"]')).toHaveCount({
        min: 1,
      });

      // Radio groups should have proper labels
      await expect(page.locator('input[type="radio"]').first()).toHaveAttribute(
        "aria-describedby",
      );
    });
  ...
  test.describe("Quote Calculator Mobile Experience", () => {
    ...
    test("should have responsive layout", async () => {
      ...
      await expect(
        page.locator("div:has(> div.w-8.h-8.rounded-full)"),
      ).toBeVisible();
  ...
  test.describe("Quote Calculator Integration", () => {
    test("should integrate with existing booking flow", async () => {
      ...
      // Modal should close
      await expect(
        page.locator("text=Selecteer Service Type"),
      ).not.toBeVisible();
    });
