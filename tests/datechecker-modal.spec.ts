...
test.describe("DateChecker Modal - Empathy-Driven Booking Flow", () => {
  ...
  test("complete booking flow reduces user anxiety through progressive steps", async ({
    page,
  }) => {
    ...
    await expect(
      page.locator("text=Selecteer uw gewenste datum voor uw evenement"),
    ).toBeVisible();
    ...
    await expect(
      page.locator("text=Hoeveel gasten mogen we verwelkomen?"),
    ).toBeVisible();
    ...
    await expect(
      page.locator("text=Reserveringsaanvraag verzonden!"),
    ).toBeVisible();
    await expect(
      page.locator("text=We bevestigen uw reservering binnen 24 uur"),
    ).toBeVisible();
  });

  test("anxiety reduction: immediate feedback and guidance", async ({
    page,
  }) => {
    // Trigger modal
    await page.evaluate(() => window.scrollBy(0, 300));
    ...
    await expect(
      page.locator("text=Beschikbaarheid controleren..."),
    ).toBeVisible();
  ...
  test("smart guidance: popular times and price transparency", async ({
    page,
  }) => {
    ...
    await expect(page.locator("text=Geschatte prijs:")).toBeVisible();
    await expect(
      page.locator("text=*Exacte prijs hangt af van uw menukeuzes"),
    ).toBeVisible();
  ...
  test("error handling and edge cases", async ({ page }) => {
    ...
    if (disabledCount > 0) {
      ...
      await expect(
        page.locator("text=Selecteer uw gewenste datum"),
      ).toBeVisible();
    }
