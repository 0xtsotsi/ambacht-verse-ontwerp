...
test.describe('Floating Booking Widget', () => {
  ...
  test('should not be visible on initial page load', async ({ page }) => {
    ...
    await expect(page.locator('[aria-label="Reserveer vandaag - Open booking formulier"]')).not.toBeVisible();
  });
  ...
  test('should appear after scrolling down', async ({ page }) => {
    ...
    const widget = page.locator('[aria-label="Reserveer vandaag - Open booking formulier"]').first();
  ...
  test('should expand when clicked in collapsed state', async ({ page }) => {
    ...
    await expect(page.locator('button:has-text("Beschikbaarheid Checken")')).toBeVisible();
  ...
  test('should scroll to booking form when "Beschikbaarheid Checken" is clicked', async ({ page }) => {
    // Trigger and expand widget
    await page.evaluate(() => window.scrollBy(0, 300));
    await page.waitForTimeout(1000);
    await page.click('button:has-text("Reserveer Vandaag")');
  ...
  test('should have proper ARIA labels and roles', async ({ page }) => {
    ...
    const collapsedWidget = page.locator('[aria-label="Reserveer vandaag - Open booking formulier"]').first();
    ...
    const bookingButton = page.locator('button:has-text("Beschikbaarheid Checken")');
...
test.describe('Floating Booking Widget - Mobile', () => {
  ...
  test('should be optimized for mobile viewport', async ({ page }) => {
    ...
    const widget = page.locator('.fixed.bottom-4.left-1\\/2.-translate-x-1\\/2').first();
    await expect(widget).toBeVisible();
    ...
    await expect(page.locator('text=Reserveer uw culinaire ervaring')).toBeVisible();
