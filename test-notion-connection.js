...
async function testNotionConnection() {
  ...
  try {
    ...
    if (!process.env.NOTION_API_KEY) {
      ...
      console.log("\nOr run with:");
      console.log(
        "NOTION_API_KEY=your_integration_secret_here node test-notion-connection.js",
      );
      return;
    }
    ...
    console.log(
      "🔑 API Key: " +
        ...
        "..." +
        process.env.NOTION_API_KEY.substring(
          process.env.NOTION_API_KEY.length - 4,
        ),
    );
    ...
    if (response && response.results) {
      ...
      console.log(
        `📝 Found ${response.results.length} records in initial query`,
      );
    ...
    if (error.code === "unauthorized") {
      ...
      console.error(
        "  Make sure your integration has access to this database.",
      );
    } else {
