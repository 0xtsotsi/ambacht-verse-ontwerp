      ...
      <div data-testid="service-tier-matrix" className="space-y-6">
        <div className="text-center">
          <h3 className="text-xl font-serif text-forest-green mb-2">
            Service Niveau Matrix
          </h3>
          <div data-testid="current-tier" className="hidden">
            {selectedTier}
          </div>
          <div data-testid="service-category" className="hidden">
            {serviceCategory.name}
          </div>
          <p className="text-natural-brown text-sm">
            Huidige service:{" "}
            <span className="font-medium">{serviceCategory.name}</span>
          </p>
        </div>
              ...
              <div key={tier.id} className="relative group">
                ...
                >
                  ...
                  <div className="w-full text-center">
                    <h4
                      className={`text-lg font-serif font-bold ${isSelected ? "text-white" : "text-forest-green"}`}
                    >
                      {tier.name}
                    </h4>
                    <div
                      className={`text-2xl font-bold mt-1 ${isSelected ? "text-warm-cream" : "text-accent"}`}
                    >
                      €{calculatedPrice}
                    ...
                    <div
                      className={`text-xs mt-1 ${isSelected ? "text-warm-cream/80" : "text-natural-brown"}`}
                    >
                  ...
                  {/* Description */}
                  <p
                    className={`text-sm text-center leading-relaxed ${isSelected ? "text-warm-cream/90" : "text-natural-brown"}`}
                  >
                    {tier.description}
                  </p>
                  ...
                  <div className="w-full">
                    <div
                      className={`text-xs font-medium ${isSelected ? "text-warm-cream/80" : "text-forest-green/80"}`}
                    >
                      Inclusief:
                    </div>
                    <ul
                      className={`text-xs mt-1 space-y-1 ${isSelected ? "text-warm-cream/90" : "text-natural-brown"}`}
                    >
                      {tier.features
                        .slice(0, 2)
                        .map((feature, featureIndex) => (
                          <li key={featureIndex} className="flex items-center">
                            <span
                              className={`w-1 h-1 rounded-full mr-2 ${isSelected ? "bg-warm-cream" : "bg-accent"}`}
                            ></span>
                            {feature}
                          </li>
                        ))}
                      {tier.features.length > 2 && (
                        <li
                          className={`font-medium ${isSelected ? "text-warm-cream/80" : "text-accent"}`}
                        >
