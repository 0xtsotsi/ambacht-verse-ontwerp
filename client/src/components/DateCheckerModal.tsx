...
const mockAvailability = {
  dates: {
    ...
    limited: [addDays(new Date(), 3), addDays(new Date(), 7)],
  },
  timeSlots: {
...
export function DateCheckerModal({
  open,
  onOpenChange,
  onConfirm,
  onOpenQuoteCalculator,
}: DateCheckerModalProps) {
  ...
  const isDateBooked = (date: Date) => {
    return mockAvailability.dates.booked.some(
      (bookedDate) =>
        format(date, "yyyy-MM-dd") === format(bookedDate, "yyyy-MM-dd"),
    );
  };
  ...
  const isDateLimited = (date: Date) => {
    return mockAvailability.dates.limited.some(
      (limitedDate) =>
        format(date, "yyyy-MM-dd") === format(limitedDate, "yyyy-MM-dd"),
    );
  };
        ...
        <div className="space-y-6 py-4">
          ...
          {step === 3 && selectedDate && selectedTime && (
            <div className="space-y-6">
              <div className="text-center mb-4">
                <p className="text-lg font-semibold text-forest-green">
                  {format(selectedDate, "EEEE d MMMM yyyy", { locale: nl })} om{" "}
                  {selectedTime}
                </p>
              </div>
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-2">
                  <div className="flex justify-between text-sm text-gray-600">
                    <span>Min. 10</span>
                    <span className="font-semibold text-lg text-forest-green">
                      {guestCount[0]} gasten
                    </span>
                    <span>Max. 200</span>
                  </div>
