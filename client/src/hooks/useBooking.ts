...
export function useBooking() {
  ...
  const submitBooking = useCallback(
    async (bookingData: CreateBookingData): Promise<Booking> => {
      try {
        setError(null);
  ...
  const updateStatus = useCallback(
    async (bookingId: string, status: BookingStatus): Promise<Booking> => {
      try {
        setError(null);

        const updatedBooking = await statusUpdateMutation.mutateAsync({
          id: bookingId,
          status,
        });
        if (!updatedBooking) {
  ...
  const generateQuote = useCallback(
    async (bookingId: string, quoteData: QuoteData): Promise<Quote> => {
      try {
        setError(null);
  ...
  const getBookingQuotes = useCallback(
    async (bookingId: string): Promise<Quote[]> => {
      try {
        setError(null);
  ...
  const subscribeToUpdates = useCallback(
    (bookingId: string, callback: (booking: Booking) => void) => {
