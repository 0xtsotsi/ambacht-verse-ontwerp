    ...
    <div data-testid="service-tier-comparison">
      ...
      {tiers.map((tier) => (
        <div
          ...
          className={selectedTier === tier.id ? "highlighted" : ""}
        >
          ...
          <ul>
            {tier.features.map((feature: string, index: number) => (
              <li key={index} data-testid={`feature-${tier.id}-${index}`}>
                {feature}
              </li>
            ))}
          </ul>
...
vi.mock("@/components/ServiceTierCalculator", () => ({
  ServiceTierCalculator: ({
    ...
    const totalPrice =
      serviceCategory.basePrice * tier.priceMultiplier * guestCount;
...
describe("ServiceTierSystem", () => {
  ...
  describe("Price Calculation", () => {
    ...
    it("should call onPriceUpdate when tier changes", async () => {
      ...
      const expectedPrice =
        SERVICE_CATEGORIES[0].basePrice * SERVICE_TIERS[2].priceMultiplier * 25;
  ...
  describe("Service Categories", () => {
    ...
    it("should recalculate prices when service category changes", () => {
      ...
      // Initial price
      const initialPrice =
        initialCategory.basePrice * tier.priceMultiplier * guestCount;
      expect(screen.getByTestId("total-price")).toHaveTextContent(
      ...
      const newPrice =
        newCategory.basePrice * tier.priceMultiplier * guestCount;
      expect(screen.getByTestId("total-price")).toHaveTextContent(
        `€${newPrice.toFixed(2)}`,
      );
    });
  ...
  describe("Tier Features Comparison", () => {
    it("should display features for each tier", () => {
      ...
      SERVICE_TIERS.forEach((tier) => {
        tier.features.forEach((feature, index) => {
          expect(
            screen.getByTestId(`feature-${tier.id}-${index}`),
    ...
    it("should highlight selected tier in comparison", async () => {
      ...
      expect(screen.getByTestId("comparison-luxury")).toHaveClass(
        "highlighted",
      );
    });
  });
  ...
  describe("Component Logging Integration", () => {
    it("should track component lifecycle events", () => {
      ...
      expect(loggerModule.ComponentLogger.lifecycle).toHaveBeenCalledWith(
        "ServiceTierSystem",
        "mount",
        expect.any(Object),
      );
...
describe("SERVICE_TIERS Configuration", () => {
  it("should have correct price multipliers", () => {
    ...
    expect(premiumTier?.priceMultiplier).toBe(1.0);
